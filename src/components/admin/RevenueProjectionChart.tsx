...
export default function RevenueProjectionChart({ allTransactions = [], withdrawalRequests = [] }: Props) {
  const [volumeGoal, setVolumeGoal] = useState<number>(10000);
  const [depositPercentage, setDepositPercentage] = useState<number>(60);
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 space-y-4">
              <div>
                ...
                <Input
                  type="number"
                  value={volumeGoal}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setVolumeGoal(parseFloat(e.target.value) || 0)}
              ...
              <div>
                ...
                <Slider
                  ...
                  onValueChange={(value: number[]) => setDepositPercentage(value[0])}
                  min={0}
                  max={100}
                  step={5}
                  className="w-full"
