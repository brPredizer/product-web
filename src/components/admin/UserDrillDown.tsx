...
export default function UserDrillDown({ userId, open, onClose }: Props) {
  const { data: user } = useQuery<User | undefined>({
    queryKey: ['user', userId],
    queryFn: async () => {
      const users = await (mockApi as any).entities.User.filter({ id: userId });
  ...
  const { data: positions = [] } = useQuery<Position[]>({
    ...
    queryFn: () => (mockApi as any).entities.Position.filter({ user_id: userId }, '-created_date', 50),
    enabled: !!userId && !!open
  });

  const { data: transactions = [] } = useQuery<Transaction[]>({
    queryKey: ['user-transactions', userId],
    queryFn: () => (mockApi as any).entities.Transaction.filter({ user_id: userId }, '-created_date', 50),
    enabled: !!userId && !!open
  });
        ...
        <Tabs defaultValue="overview" className="mt-6">
          ...
          <TabsContent value="overview" className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <InfoCard
                title="Mercados"
                value={String(user.markets_participated || 0)}
                icon={TrendingUp}
              />
            </div>
            ...
            <div className="text-sm text-slate-500 space-y-1">
              <p>Cadastrado em: {user.created_date ? format(new Date(user.created_date), "d 'de' MMMM 'de' yyyy", { locale: ptBR }) : '-'}</p>
              <p>ID: {user.id}</p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="activity" className="space-y-4 mt-4">
            <div className="space-y-3">
              <InfoCard
                ...
                value={`${Math.round(user.win_rate || 0)}%`}
                icon={TrendingUp}
              />
              <InfoCard
