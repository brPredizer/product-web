  ...
  const generateForecasts = async () => {
    ...
    try {
      ...
      transactions.forEach(tx => {
        ...
        if (tx.type === 'deposit') {
          dailyMetrics[date].deposits += (tx.amount || 0);
          dailyMetrics[date].revenue += (tx.fee || 0);
      ...
      // Calcular churn (usuários inativos por 5+ dias)
      const calculateChurn = () => {
        ...
        return users.filter(user => {
          ...
          const lastTx = userTxs.sort((a, b) => 
            new Date(b.created_date) - new Date(a.created_date)
          )[0];
          
          return new Date(lastTx.created_date) <= fiveDaysAgo;
        }).length;
      };
      ...
      const prediction = await mockApi.integrations.Core.InvokeLLM({
...
Seja conservador nas previsões, considerando volatilidade do mercado.`,
  ...
  const ForecastCard = ({ title, icon: Icon, prediction, currentValue, format = 'number' }) => {
    ...
    const growth7d = ((prediction.next_7_days - currentValue) / currentValue * 100);
    const growth30d = ((prediction.next_30_days - currentValue) / currentValue * 100);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Previsões baseadas em dados históricos e tendências de mercado
          </p>
        </div>
        <Button
      ...
      {loading && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-600 font-medium">Analisando {users.length} usuários e {transactions.length} transações...</p>
      ...
      {forecasts && !loading && (
        <>
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            {forecasts.opportunities?.length > 0 && (
          ...
          {/* Recommendations */}
          {forecasts.recommendations?.length > 0 && (
      ...
      {!forecasts && !loading && (
        <div className="text-center py-12 bg-slate-50 rounded-xl border-2 border-dashed border-slate-300">
          ...
          <p className="text-sm text-slate-500 mb-4">
            Use IA para prever tendências futuras baseadas em dados históricos
          </p>
        </div>
      )}
    </div>
