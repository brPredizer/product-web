...
const segmentConfig: Record<string, { icon: any; color: string; description: string }> = {
  'high_value': {
    icon: DollarSign,
...
export default function UserSegmentation({ users, transactions, positions }: Props) {
    ...
    try {
      const userMetrics = users.map((user) => {
        ...
        const daysSinceCreation = Math.floor(
          (Date.now() - new Date(user.created_date || Date.now()).getTime()) / (1000 * 60 * 60 * 24)
        );
        ...
        const daysSinceLastActivity = lastTransaction
          ? Math.floor((Date.now() - new Date(lastTransaction.created_date || Date.now()).getTime()) / (1000 * 60 * 60 * 24))
          : daysSinceCreation;
      ...
      // Chamada ao mockApi (tipado como any para compatibilidade gradual)
      const analysis = await (mockApi as any).integrations.Core.InvokeLLM({
...
${JSON.stringify(userMetrics.slice(0, 100), null, 2)}`,
      ...
      setSegments((analysis && analysis.segments) || null);
    ...
    <div className="space-y-6">
        ...
        <>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(segments).map(([key, segment]) => {
              const config = segmentConfig[key] || segmentConfig['active_traders'];
          ...
          {selectedSegment && segments[selectedSegment] && (
            <Card className="border-2 border-purple-200 bg-purple-50/50">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2">
                    {React.createElement(segmentConfig[selectedSegment]?.icon || Target, { className: "w-5 h-5 text-purple-600" })}
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-700 mb-2">Usuários no Segmento ({segments[selectedSegment].count || 0}):</p>
                  <div className="bg-white rounded-lg p-4 border border-slate-200 max-h-48 overflow-y-auto">
                    <div className="space-y-1">
                      ...
                      {(segments[selectedSegment].user_ids || []).length > 20 && (
                        <p className="text-xs text-slate-500 text-center pt-2">... e mais {(segments[selectedSegment].user_ids || []).length - 20} usuários</p>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {!segments && !loading && (
        <div className="text-center py-12 bg-slate-50 rounded-xl border-2 border-dashed border-slate-300">
          ...
          <p className="text-sm text-slate-500 mb-4">Clique em "Executar Segmentação" para analisar seus usuários com IA</p>
        </div>
      )}
    </div>
  );
