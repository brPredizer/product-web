...
export default function AIMarketingAssistant({ markets, contentPosts, onCreatePost, user, adminLevel }) {
  ...
  const createDraft = async () => {
    ...
    try {
      ...
      await onCreatePost?.({
        ...
        status: 'review', // Status de revisão
        metadata: {
  ...
  // 2. Carregar Rascunhos Pendentes
  ...
  const approvePost = async (post) => {
    try {
      ...
      // Criar mercado se houver valor e posição
      ...
      if (metadata?.value) {
        ...
        await mockApi.entities.Market.create({
          ...
          resolution_source: 'Análise da equipe PredictX',
  ...
  // 4. Rejeitar Rascunho
  const rejectPost = async (post, reason) => {
    try {
      setLoading(true);
  ...
  const canApprove = adminLevel >= 2; // Níveis 2 e 3 podem aprovar

  return (
    <div className="grid md:grid-cols-2 gap-4">
      {/* Criar Rascunho de Postagem */}
      <Dialog open={draftDialogOpen} onOpenChange={setDraftDialogOpen}>
        <DialogTrigger asChild>
          <Card className="p-6 hover:shadow-lg transition-all group cursor-pointer">
            ...
            <p className="text-sm text-slate-600 mb-4">
              Crie rascunhos de posts com análises e probabilidades para aprovação.
            </p>
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              O rascunho será enviado para aprovação antes de ser publicado
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            {draftValue && (
              <p className="text-xs text-slate-500 text-center">
                ✨ Após aprovação, um mercado votável será criado com {draftValue}% para {draftPosition === 'yes' ? 'SIM' : 'NÃO'}
              </p>
            )}
          </div>
        </DialogContent>
        ...
        <Dialog open={approvalDialogOpen} onOpenChange={setApprovalDialogOpen}>
            ...
            <Card className="p-6 hover:shadow-lg transition-all group cursor-pointer" onClick={loadPendingPosts}>
              <div className="flex items-start justify-between mb-4">
                ...
                <Badge className="bg-purple-100 text-purple-700">
                  {contentPosts.filter(p => p.status === 'review').length} pendentes
                </Badge>
              </div>
              <h3 className="text-lg font-semibold text-slate-900 mb-2">Aprovar Rascunhos</h3>
              <p className="text-sm text-slate-600 mb-4">
                Revise e aprove ou rejeite rascunhos criados pela equipe.
              </p>
              <Button 
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Revise os rascunhos e aprove ou rejeite para devolução
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
