...
const ChartContext = React.createContext<any | null>(null)
...
export const ChartContainer = React.forwardRef<any, any>(({ id, className, children, config = {}, ...props }, ref) => {
...
export const ChartLegendContent = React.forwardRef<any, any>(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  const { config } = useChart()
  ...
  return (
    <div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item: any) => {
        ...
        return (
          <div key={item.value ?? item.dataKey} className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground")}>{
...
// Named exports are declared above; no additional export list required.
