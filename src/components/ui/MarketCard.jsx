...
export default function MarketCard({ market, compact = false }) {
  ...
  const isClosingSoon = market.closing_date && 
    new Date(market.closing_date) - new Date() < 24 * 60 * 60 * 1000;
      ...
      <div className="p-5">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-2">
            <span className="font-semibold text-emerald-600">SIM R$ {(market.yes_price).toFixed(2)}</span>
            <span className="font-semibold text-rose-500">NÃO R$ {(market.no_price || (1 - market.yes_price)).toFixed(2)}</span>
          </div>
        ...
        <div className="flex items-center justify-between text-xs text-slate-500">
          <div className="flex items-center gap-1">
            <Users className="w-3.5 h-3.5" />
            <span>R$ {((market.volume_total || 0) / 1000).toFixed(1)}k volume</span>
          </div>
          {market.closing_date && (
            <span>
