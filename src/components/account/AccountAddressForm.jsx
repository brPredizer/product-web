      ...
      <CardContent className="space-y-4">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (digits.length === 8) {
                  ...
                  cepDebounceRef.current = setTimeout(() => lookupCep(digits), 450);
                }
              }}
              onBlur={() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <Input value={address.street} onChange={(e) => setAddress((p) => ({ ...p, street: e.target.value }))} className="mt-1" placeholder="Rua / Avenida" />
          </div>
          <div>
            <label className="text-sm text-slate-700">Número</label>
            <Input value={address.number} onChange={(e) => setAddress((p) => ({ ...p, number: e.target.value }))} className="mt-1" placeholder="Numero" />
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-slate-700">Bairro</label>
            <Input value={address.neighborhood} onChange={(e) => setAddress((p) => ({ ...p, neighborhood: e.target.value }))} className="mt-1" placeholder="Bairro" />
          </div>
          <div>
            <label className="text-sm text-slate-700">Complemento</label>
            <Input value={address.complement} onChange={(e) => setAddress((p) => ({ ...p, complement: e.target.value }))} className="mt-1" placeholder="Apto, bloco, sala..." />
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Input value={address.state} onChange={(e) => setAddress((p) => ({ ...p, state: e.target.value.toUpperCase().slice(0, 2) }))} className="mt-1" placeholder="SP" />
          </div>
        </div>
