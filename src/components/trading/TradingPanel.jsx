...
export default function TradingPanel({
  ...
  // Check if user has valid risk disclosure
  ...
  const createDisclosureMutation = useMutation({
    mutationFn: (data) => mockApi.entities.RiskDisclosure.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['risk-disclosures', userId] });
    }
  });

  const hasValidDisclosure = disclosures.some(d => {
  ...
  const potentialPayout = units * 1; // Cada contrato paga R$1 se acertar
  const potentialProfit = potentialPayout - (parseFloat(amount) || 0);

  const handleTrade = async () => {
    if (!amount || parseFloat(amount) <= 0) return;
    
    // Check if user has accepted risk disclosure
      ...
      >
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1">
            ...
            <span className={cn(
              ...
              side === 'yes' ? "text-emerald-600" : "text-slate-700"
            )}>SIM</span>
          </span>
        ...
        <div className="mt-1 flex items-baseline justify-between">
          <span className={cn(
            ...
            "font-bold",
            side === 'yes' ? "text-emerald-600" : "text-slate-900"
          )}>
      ...
      >
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1">
            ...
            <span className={cn(
              "text-sm font-semibold",
              side === 'no' ? "text-rose-600" : "text-slate-700"
            )}>NÃO</span>
          </span>
          {!isEmbedded && <span className="text-xs text-slate-500">{Math.round(market.no_price * 100)}%</span>}
        </div>
        <div className="mt-1 flex items-baseline justify-between">
          <span className={cn(
            isEmbedded ? "text-base" : "text-xl",
            "font-bold",
            side === 'no' ? "text-rose-600" : "text-slate-900"
          )}>
      ...
      <div className={bodyClass}>
        ...
        <div className="flex items-start gap-2 mt-4 p-3 bg-amber-50 rounded-lg">
          ...
          <p className="text-xs text-amber-700">
            Cada contrato paga R$1,00 se você acertar o resultado; caso contrário, paga R$0,00. 
            Você pode perder 100% do valor investido.
          </p>
        </div>
        </div>
      </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700">
                  Antes de realizar sua primeira aposta, você precisará ler e aceitar o Termo de Ciência e Risco.
                </p>
              </div>
            </div>
          </div>
