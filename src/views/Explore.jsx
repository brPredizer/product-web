...
export default function Explore({ user }) {
  ...
  const [selectedCategories, setSelectedCategories] = useState([]); // agora 0 ou 1 categoria
  const [sort, setSort] = useState("-volume_total");
  const [viewMode, setViewMode] = useState("grid");
  const [filtersOpen, setFiltersOpen] = useState(false);

  const [probabilityFilters, setProbabilityFilters] = useState([]);
  const [closingRange, setClosingRange] = useState("any");
  const [statusFilters, setStatusFilters] = useState([]); // 'new', 'trending'

  // ✅ Lê category da URL e aplica
  useEffect(() => {
  ...
  // ✅ Setar categoria + sincronizar URL
  ...
  const isCategoryActive = (id) =>
    id === "all" ? selectedCategories.length === 0 : selectedCategories.includes(id);
  ...
  const { data: markets = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const data = await mockApi.entities.Market.filter({ status: "open" }, sort, 100);
      return data;
    },
  });
    ...
    return markets
      .filter((market) => {
        ...
        const isNew = (() => {
          ...
          const diffDays =
            (now - new Date(market.created_date).getTime()) / (1000 * 60 * 60 * 24);
          return diffDays <= 7;
        })();
      ...
      .sort((a, b) => {
        ...
        if (sort === "closing_date") return new Date(a.closing_date) - new Date(b.closing_date);
        if (sort === "-yes_price") return (b.yes_price || 0) - (a.yes_price || 0);
        if (sort === "yes_price") return (a.yes_price || 0) - (b.yes_price || 0);
        if (sort === "-imbalance") {
          const imbalance = (m) => Math.abs((m.yes_price ?? 0.5) - 0.5);
  ...
  const clearAllFilters = () => {
    ...
    // limpa querystring de category
    const params = new URLSearchParams(searchParams.toString());
    params.delete("category");
    router.replace(`/Explore?${params.toString()}`, { scroll: false });
  };

  const showCreateAction = user?.role === "admin";
