...
export default function WalletPage({ user, refreshUser }) {
  ...
  const { data: transactions = [], isLoading } = useQuery({
    ...
    queryFn: () => mockApi.entities.Transaction.filter({ user_id: user?.id }, '-created_date', 50),
    enabled: !!user?.id
  });

  const depositMutation = useMutation({
    mutationFn: async (depositAmount) => {
      const fee = depositAmount * DEPOSIT_FEE;
      ...
      await mockApi.entities.Transaction.create({
        ...
        description: `Depósito via PIX (taxa: ${(DEPOSIT_FEE * 100).toFixed(0)}%)`
      });
  ...
  const withdrawMutation = useMutation({
    mutationFn: async (withdrawAmount) => {
      ...
      await mockApi.entities.WithdrawalRequest.create({
        user_id: user.id,
        ...
        status: 'pending',
        payment_method: 'PIX',
        payment_details: 'Chave PIX cadastrada'
      });

      // Atualizar saldo do usuário (bloquear valor)
      await mockApi.auth.updateMe({
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <Dialog open={depositOpen} onOpenChange={setDepositOpen}>
                ...
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Adicione saldo à sua conta via PIX
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <DialogFooter>
                    ...
                    <Button 
                      ...
                      disabled={depositAmount < 10 || depositMutation.isPending}
              ...
              <Dialog open={withdrawOpen} onOpenChange={setWithdrawOpen}>
                ...
                <DialogContent className="sm:max-w-md">
                  ...
                  <div className="space-y-6 py-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-500 mt-2">
                        Saldo disponível: R$ {(user.balance || 0).toFixed(2)}
                      </p>
                    </div>
                  ...
                  <DialogFooter>
                    ...
                    <Button 
                      className="bg-emerald-600 hover:bg-emerald-700"
                      disabled={withdrawAmount < 10 || withdrawAmount > (user.balance || 0) || withdrawMutation.isPending}
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-8">
          <StatsCard
            ...
            value={`R$ ${(user.total_deposited || 0).toFixed(2)}`}
            icon={ArrowDownCircle}
          />
          <StatsCard
            title="Total Sacado"
            value={`R$ ${(user.total_withdrawn || 0).toFixed(2)}`}
            icon={ArrowUpCircle}
          />
          <StatsCard
    ...
    <div className="p-4 sm:p-6 flex items-center gap-4 hover:bg-slate-50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-400 mt-1">
          {format(new Date(transaction.created_date), "d 'de' MMM, HH:mm", { locale: ptBR })}
        </p>
      </div>
      ...
      <div className="text-right">
        ...
        {transaction.status === 'pending' && (
          <Badge className="bg-amber-100 text-amber-700 mt-1">Pendente</Badge>
        )}
      </div>
    </div>
  );
