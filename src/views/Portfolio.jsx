...
export default function Portfolio({ user }) {
  ...
  const { data: positions = [], isLoading } = useQuery({
    ...
    queryFn: () => mockApi.entities.Position.filter({ user_id: user?.id }, '-created_date', 100),
    enabled: !!user?.id
  });
  ...
  const totalInvested = openPositions.reduce((sum, p) => sum + (p.total_invested || 0), 0);
  const totalPnL = closedPositions.reduce((sum, p) => sum + (p.pnl || 0), 0);
  const winCount = closedPositions.filter(p => p.status === 'won').length;
...
function PositionCard({ position, closed }) {
