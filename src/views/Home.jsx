...
function exploreUrlWithCategory(categoryId) {
...
export default function Home() {
  ...
  const { data: markets = [], isLoading } = useQuery({
    ...
    queryFn: () => mockApi.entities.Market.filter({ status: "open" }, "-volume_total", 50),
  });
  ...
  const featuredMarkets = useMemo(
    () => markets.filter((m) => m.featured).slice(0, 3),
    [markets]
  );

  const trendingMarkets = useMemo(() => {
    const filtered =
      activeCategory === "all" ? markets : markets.filter((m) => m.category === activeCategory);
