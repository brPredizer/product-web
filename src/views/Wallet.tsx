...
const InvisibleScrollArea = React.forwardRef<HTMLDivElement, {
  className?: string;
...
export default function WalletPage({ user, refreshUser }: WalletProps) {
  ...
  const cardMethods = useMemo(
    () => (paymentMethods || []).filter((method: any) => method.type === "CARD"),
    [paymentMethods]
  );
  ...
  const depositAmount = toNumberSafe(depositAmountStr);
  const depositFee = depositAmount * DEPOSIT_FEE;
  const depositNet = Math.max(0, depositAmount - depositFee);

  const withdrawAmount = toNumberSafe(withdrawAmountStr);
  const withdrawFee = withdrawAmount * WITHDRAWAL_FEE;
  const withdrawNet = Math.max(0, withdrawAmount - withdrawFee);

  const depositMutation = useMutation({
    mutationFn: async ({ amount }: { amount: number }) => {
      return await walletClient.createDepositIntent({ amount });
    },
    onSuccess: (data: any) => {
      const normalized = normalizePixResponse(data?.pix || null);
  ...
  const withdrawMutation = useMutation({
    mutationFn: async (amount: number) => {
      await walletClient.createWithdrawal({ amount });
    },
    onSuccess: () => {
  ...
  // Timer de expiração (contador)
  useEffect(() => {
    let timer: any = null;
    const checkedRef = { called: false } as { called: boolean };
    ...
    if (pixData?.expiresAt) {
      ...
      const update = () => {
        ...
        setExpiresIn(secs);
        if (secs <= 0 && !checkedRef.called) {
          checkedRef.called = true;
          if (timer) clearInterval(timer);
          // Uma única checagem no backend ao expirar
          (async () => {
            try {
              const paymentId = pixData?.paymentId;
              if (!paymentId) return;
              const statusRes = await paymentsClient.getMercadoPagoStatus(paymentId as any);
              const backendStatus = (statusRes?.status || '').toLowerCase();

              if (backendStatus) {
                // Normaliza status do MercadoPago para os status usados no front
                const mp = (backendStatus || '').toLowerCase();
                ...
                const normalized = mapStatus(mp);
                // Atualiza o status conforme backend normalizado
                setPixData((prev: any) => (prev ? { ...prev, status: normalized } : prev));

                // Mostrar toast único conforme o novo status normalizado
                if (normalized === 'approved') {
                  toast.success('Pagamento confirmado. Saldo será atualizado em breve.');
                } else if (normalized === 'rejected') {
                  toast.error('Pagamento recusado/cancelado. Gere um novo PIX.');
                } else if (normalized === 'expired') {
                  toast.info('PIX expirado. Gere um novo pagamento.');
                } else if (normalized === 'pending') {
                  // opcional: notificamos que continua pendente (silencioso por padrão)
                }

                // Se backend indicar estado que não deve mais expor o QR, limpa
                const removedStates = ['expired', 'rejected'];
                if (removedStates.includes(normalized)) {
                  setPixData((prev: any) =>
                ...
                // Sem status, marca como expirado localmente e notifica
                setPixData((prev: any) =>
              ...
              // Em caso de erro, apenas marca como expirado localmente
              setPixData((prev: any) =>
                prev ? { ...prev, status: 'expired', qrCode: null, qrBase64: null } : prev
              );
              setQrImage(null);
  ...
  // ref para controlar scroll do modal PIX (para rolar até o final quando abrir)
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // esperar o render do conteúdo antes de rolar
    const t = setTimeout(() => {
      try {
    ...
    async function generate() {
      ...
      if (pixData.qrCode) {
        try {
          ...
          const dataUrl = await QRCode.toDataURL(pixData.qrCode, {
            margin: 2,
            scale: 9,
          });
  ...
  const copyPix = async () => {
    ...
    if ((pixData?.status || '').toLowerCase() === 'expired' || (expiresIn ?? 0) <= 0) {
      return toast.error('PIX expirado. Gere um novo pagamento.');
    }
    await navigator.clipboard?.writeText(code);
    toast.success("PIX copiado");
  };

  const invalidatePix = async () => {
    if (!pixData) return;
    const ok = window.confirm('Deseja invalidar este PIX e remover o código/QR localmente?');
    if (!ok) return;

    // Limpa localmente para evitar uso do QR expirado
    setPixData((prev: any) =>
      prev ? { ...prev, status: 'expired', qrCode: null, qrBase64: null } : prev
    );
    setQrImage(null);
    ...
    // Atualiza queries relacionadas para refletir estado no UI
    queryClient.invalidateQueries({ queryKey: ['wallet-balances', user?.id] });
    queryClient.invalidateQueries({ queryKey: ['wallet-ledger', user?.id] });

    toast.success('PIX invalidado localmente.');
  };
                ...
                <DialogContent className="w-[min(88vw,720px)] max-h-[92vh] overflow-hidden p-0 focus:outline-none focus-visible:outline-none focus:ring-0 ring-0">
                  <div className="border-b border-slate-100 p-5">
                    <DialogHeader>
                      ...
                      <DialogDescription>Escolha o método e gere o pagamento.</DialogDescription>
                    </DialogHeader>
                  </div>

                  <InvisibleScrollArea className="max-h-[calc(92vh-72px-96px)] pt-1 pb-5 px-5 space-y-5">
                    <div>
                      ...
                      {depositMethod === "CARD" && (
                        <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
                          {cardMethods.length > 0
                            ? `Você tem ${cardMethods.length} cartão(ões) salvos. (Integração do débito/crédito aqui)`
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          onChange={(e: any) =>
                            setDepositAmountStr(clampMoneyInput(e.target.value))
                          }
                          inputMode="decimal"
                          placeholder="0,00"
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 flex gap-3">
                      ...
                      <div className="text-sm text-slate-600">
                        Ao gerar, você recebe um QR Code e o código copia e cola.
                      </div>
                    </div>

                    {/* footer dentro do scroll: botões acompanham o conteúdo */}
                    <div className="mt-2 border-t border-slate-100 pt-4 flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={
                          ...
                          (depositMethod === "CARD" && cardMethods.length === 0)
                        }
                        onClick={() => {
                          if (depositMethod === "CARD") {
                            toast("Cartão: pluga a integração aqui (PaymentIntent/Checkout).");
                            return;
                          }
...
{/* Modal 2: PIX (1 coluna, mais largo, sem cards cortando, paymentId antes do QR, taxas no final) */}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 mb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <Button
            ...
            disabled={
              !pixData?.qrCode ||
              (pixData?.status || '').toLowerCase() === 'expired' ||
              (expiresIn ?? 0) <= 0
            }
          >
          ...
          {/* botão de invalidar removido — invalidação automática ocorre ao expirar */}
        </div>
        ...
        <div className="mt-4 flex items-center justify-center">
          {qrImage ? (
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 w-full">
              {(pixData?.status || '').toLowerCase() === 'expired' || (expiresIn ?? 0) <= 0 ? (
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-col gap-2">
                ...
                <TooltipProvider delayDuration={120}>
                  <Tooltip>
                    ...
                    >
                      ...
                      <p>
                        Cobramos <strong>2,5%</strong> para cobrir custos do gateway e processamento do pagamento.
            ...
            <div className="flex items-baseline justify-between w-full">
              ...
              <span className="font-bold font-medium text-emerald-600 tabular-nums whitespace-nowrap text-sm">
                R$ {depositNet.toFixed(2)}
              </span>
            </div>
          </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          onChange={(e: any) =>
                            setWithdrawAmountStr(clampMoneyInput(e.target.value))
                          }
                          inputMode="decimal"
                          placeholder="0,00"
                          className="pl-10 h-12 text-lg"
                    ...
                    {withdrawAmount > 0 && (
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-3 border-t pt-3 flex justify-between">
                          ...
                          <span className="font-bold text-emerald-600 font-medium">
