...
export default function Admin({ user }) {
  ...
  const [kpiDetailModal, setKpiDetailModal] = useState({ open: false, type: null, data: null });
  ...
  const handlePromoteToAdmin = async (userId, userEmail, level) => {
    const levelNames = {
    ...
    try {
      await mockApi.entities.User.update(userId, { role: 'admin', admin_level: level });
  ...
  const handleChangeAdminLevel = async (userId, userEmail, currentLevel, newLevel) => {
    const levelNames = {
      1: 'Nível 1 (Marketing/Publicidade)',
      2: 'Nível 2 (Responsabilidade/IA)',
      3: 'Nível 3 (Acesso Total)'
    ...
    try {
      await mockApi.entities.User.update(userId, { admin_level: newLevel });
      await queryClient.invalidateQueries(['admin-users']);
  ...
  const handleApproveWithdrawal = async (withdrawal) => {
    ...
    try {
      ...
      // Atualizar total sacado do usuário
      const userToUpdate = allUsers.find(u => u.id === withdrawal.user_id);
      if (userToUpdate) {
  ...
  const handleDeleteUser = async (userId, userEmail) => {
    ...
    try {
      ...
      // Invalidar todas as queries para atualizar os números
      await queryClient.invalidateQueries(['admin-users']);
      await queryClient.invalidateQueries(['admin-transactions']);
  ...
  const handleRevertDeposit = async (transactionId, userId, amount, netAmount, fee) => {
    ...
    try {
      ...
      // Invalidar queries para atualizar KPIs
      await queryClient.invalidateQueries(['admin-users']);
      await queryClient.invalidateQueries(['admin-transactions']);
      ...
      toast.dismiss();
      toast.error('Erro ao reverter depósito: ' + error.message);
    }
  };

  const handleResetUserData = async (userId, userEmail) => {
    ...
    try {
      ...
      // Resetar dados financeiros e estatísticas do usuário
      await mockApi.entities.User.update(userId, {
        balance: 0,
        total_deposited: 0,
        total_withdrawn: 0,
  ...
  // Determine admin level (default to 3 for existing admins without level)
  const adminLevel = user.admin_level || 3;

  // Permission checks
  const canAccessAdmin = adminLevel === 3; // Nível 3: Administração, Saques/Depósitos
  const canAccessMarketing = adminLevel >= 1; // Níveis 1, 2, 3: Marketing, Publicidade
  const canAccessResponsibility = adminLevel >= 2; // Níveis 2, 3: Responsabilidade, Segmentação, Previsões
  const canAccessSegmentation = adminLevel >= 2;
  const canAccessPredictive = adminLevel >= 2;
  ...
  const activeUsers = allUsers.filter(u => (u.balance || 0) > 0 || (u.markets_participated || 0) > 0).length;

  // ==================== CÁLCULOS FINANCEIROS REVISADOS ====================
  
  // DEPÓSITOS E SAQUES - HISTÓRICO COMPLETO
  ...
  // Total BRUTO depositado pelos usuários (antes das taxas)
  const totalDepositedGross = allDeposits.reduce((sum, t) => sum + (t.amount || 0), 0);
  
  // Total LÍQUIDO depositado (creditado aos usuários após taxa de 7%)
  const totalDepositedNet = allDeposits.reduce((sum, t) => sum + (t.net_amount || 0), 0);
  
  // ==================== SAQUES - USAR APENAS WITHDRAWALREQUESTS ====================
  
  // Apenas saques aprovados/completados
  const allApprovedWithdrawals = withdrawalRequests.filter(w => w.status === 'approved' || w.status === 'completed');
  
  // Total BRUTO Sacado (antes da taxa de 10%)
  const totalWithdrawnGross = allApprovedWithdrawals.reduce((sum, w) => sum + (w.amount || 0), 0);
  
  // Total LÍQUIDO Sacado (após taxa de 10%)
  const totalWithdrawnNetFinalCorrect = allApprovedWithdrawals.reduce((sum, w) => sum + (w.net_amount || 0), 0);
  
  // Total de Taxas de Saque (10%)
  const allApprovedWithdrawalFees = allApprovedWithdrawals.reduce((sum, w) => sum + (w.fee || 0), 0);
  
  // CUSTÓDIA = Saldo disponível de todos os usuários (nossa responsabilidade)
  const totalCustody = allUsers.reduce((sum, u) => sum + (u.balance || 0), 0);
  const avgBalance = totalUsers > 0 ? totalCustody / totalUsers : 0;
  
  console.log('=== AUDITORIA FINANCEIRA ===');
  console.log('Total Bruto Depositado:', totalDepositedGross.toFixed(2));
  console.log('Total Líquido Depositado (creditado):', totalDepositedNet.toFixed(2));
  console.log('Total Bruto Sacado:', totalWithdrawnGross.toFixed(2));
  console.log('Total Líquido Sacado (pago):', totalWithdrawnNetFinalCorrect.toFixed(2));
  console.log('Taxa Saque (10%):', allApprovedWithdrawalFees.toFixed(2));
  console.log('Custódia Atual (saldo usuários):', totalCustody.toFixed(2));
  ...
  // Volume Total = Soma de todas as transações de compra (buy)
  ...
  // Taxas de DEPÓSITO (7%) - histórico completo
  const allDepositFees = allDeposits.reduce((sum, t) => sum + (t.fee || 0), 0);
  
  // Taxas de SAQUE (10%) - usar diretamente dos WithdrawalRequests
  ...
  console.log('Taxas de Depósito (7%):', allDepositFees.toFixed(2));
  console.log('Taxas de Saque (10%):', totalWithdrawalFees.toFixed(2));
  console.log('Receita Total:', totalPlatformRevenueAllTime.toFixed(2));
  console.log('Receita no Período:', totalPlatformRevenue.toFixed(2));
  ...
  // Risk & Responsibility KPIs
  const validDisclosures = riskDisclosures.filter(d => {
    if (d.revoked) return false;
    const validUntil = new Date(d.valid_until);
    return validUntil > new Date();
  });
  const usersWithValidDisclosure = new Set(validDisclosures.map(d => d.user_id)).size;
  ...
  const usersOnBreak = userRiskLimits.filter(l => l.trading_break_until && new Date(l.trading_break_until) > new Date()).length;
  const usersExcluded = userRiskLimits.filter(l => l.self_excluded_until && new Date(l.self_excluded_until) > new Date()).length;
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs defaultValue={canAccessAdmin ? "admin" : canAccessMarketing ? "marketing" : "responsibility"} className="space-y-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <KPICard
                  ...
                  onDrillDown={() => setKpiDetailModal({
                    ...
                    type: 'totalCustody',
                    data: {
                      users: allUsers.sort((a, b) => (b.balance || 0) - (a.balance || 0)),
                      total: totalCustody,
                      count: allUsers.filter(u => (u.balance || 0) > 0).length
                    }
                  })}
                />
              </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <KPICard
                  ...
                  onDrillDown={() => setKpiDetailModal({
                    ...
                    data: {
                      transactions: allDeposits.sort((a, b) => new Date(b.created_date) - new Date(a.created_date)),
                      users: allUsers,
                ...
                <KPICard
                  ...
                  onDrillDown={() => setKpiDetailModal({
                    ...
                    data: {
                      withdrawals: allApprovedWithdrawals.sort((a, b) => new Date(b.created_date) - new Date(a.created_date)),
                        ...
                        <tr key={u.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex gap-2 flex-wrap">
                              ...
                              {u.role !== 'admin' && (
                                <Select onValueChange={(level) => handlePromoteToAdmin(u.id, u.email, parseInt(level))}>
                              ...
                              {u.role === 'admin' && u.id !== user.id && adminLevel === 3 && (
                                <Select value={String(u.admin_level || 3)} onValueChange={(level) => handleChangeAdminLevel(u.id, u.email, u.admin_level || 3, parseInt(level))}>
                                  <SelectTrigger className="w-32 h-8 text-xs">
                                    <SelectValue />
                                  </SelectTrigger>
                                  <SelectContent>
          ...
          {/* WITHDRAWALS TAB */}
          {canAccessAdmin && <TabsContent value="withdrawals" className="space-y-8">
            ...
            <section>
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {withdrawalRequests.length > 0 ? withdrawalRequests.map(withdrawal => (
                  ...
                  <tfoot className="bg-gradient-to-r from-blue-50 to-cyan-50 border-t-2 border-blue-200">
                    <tr>
                      ...
                      <td className="px-6 py-4 text-sm font-bold text-slate-900">
                        R$ {withdrawalRequests.reduce((sum, w) => sum + (w.amount || 0), 0).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 text-sm font-bold text-rose-700">
                        R$ {withdrawalRequests.reduce((sum, w) => sum + (w.fee || 0), 0).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 text-sm font-bold text-emerald-700">
                        R$ {withdrawalRequests.reduce((sum, w) => sum + (w.net_amount || 0), 0).toFixed(2)}
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-500">
            ...
            <section>
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {riskDisclosures.slice(0, 20).map(disclosure => {
                      const isValid = !disclosure.revoked && new Date(disclosure.valid_until) > new Date();
                      const userEmail = allUsers.find(u => u.id === disclosure.user_id)?.email || 'Usuário';
            ...
            <section>
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {userRiskLimits.slice(0, 20).map(limit => {
                      const userEmail = allUsers.find(u => u.id === limit.user_id)?.email || 'Usuário';
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {allUsers
                      .sort((a, b) => (b.total_wagered || 0) - (a.total_wagered || 0))
                      .map(u => {
                        const userTxCount = allTransactions.filter(t => t.user_id === u.id).length;
                        const userPosCount = allPositions.filter(p => p.user_id === u.id).length;
                    ...
                    {allUsers
                      ...
                      .map(u => {
                        ...
                        const userDeposits = allTransactions
                          .filter(t => t.user_id === u.id && t.type === 'deposit')
